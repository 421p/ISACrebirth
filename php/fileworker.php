...
function danceWithJson($json_string, $delimiter, $filename, $taskDir){
    ...
    for($i = 0; $i < 5; $i++){
        ...
        if(substr_count($wArray[$i]['input'], $delimiter) != substr_count($wArray[$i]['output'], $delimiter)) die ('Error. Delimiters.');
        if(substr_count($wArray[$i]['examples']['input'], $delimiter) != substr_count($wArray[$i]['examples']['output'], $delimiter)) die ('Error. Delimiters.');
